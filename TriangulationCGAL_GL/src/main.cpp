...
void DisplayGL() {
	...
	glDrawArrays(GL_TRIANGLES, 0, vertexDV.size());
...
int main(int argc, char ** argv) {
	...
	for (int i = 0; i < polygons.size(); i++) {
		...
		for (int j = 0; j < polygons[i].num_vertex; j++) {
			polygon.push_back(
		...
		for (poly_it = convex_partition_polys.begin();
			...
			assert(
					CGAL::is_y_monotone_2((*poly_it).vertices_begin(),
			...
			for (Finite_faces_iterator it = rt.finite_faces_begin();
				...
				std::cout << " " << rt.triangle(it)[0].x() << " "
						<< rt.triangle(it)[0].y() << " ";
				std::cout << " " << rt.triangle(it)[1].x() << " "
						<< rt.triangle(it)[1].y() << " ";
				std::cout << " " << rt.triangle(it)[2].x() << " "
						<< rt.triangle(it)[2].y() << " ";
				std::cout << std::endl << "-------------------" << std::endl;

				VertexXYZColor vertexGL1;
				VertexXYZColor vertexGL2;
				VertexXYZColor vertexGL3;
				vertexGL1.m_Color = glm::vec3(0.0f, 0.0f, 1.0f);
				vertexGL1.m_Pos = glm::vec3(rt.triangle(it)[0].x(),
						rt.triangle(it)[0].y(), 0.0f);
				vertexGL2.m_Color = glm::vec3(0.0f, 0.0f, 1.0f);
				vertexGL2.m_Pos = glm::vec3(rt.triangle(it)[1].x(),
						rt.triangle(it)[1].y(), 0.0f);
				vertexGL3.m_Color = glm::vec3(0.0f, 0.0f, 1.0f);
				vertexGL3.m_Pos = glm::vec3(rt.triangle(it)[2].x(),
						rt.triangle(it)[2].y(), 0.0f);

				vertexDV.push_back(vertexGL1);
				vertexDV.push_back(vertexGL2);
				vertexDV.push_back(vertexGL3);
	...
	/*for (int i = 0; i < polygons.size(); i++) {
	 ...
	 for (int j = 0; j < polygons[i].num_vertex; j++) {
	 Regular_triangulation::Weighted_point wp;
	 wp = Regular_triangulation::Weighted_point(polygons[i].vertex[j].x,
