...
static INLINE void quat_to_mat(mat4_t res, quat_t q) {
	m4_cons(res, 1.0f - 2.0f * q.y * q.y - 2.0f * q.z * q.z,
			2.0f * q.x * q.y + 2.0f * q.w * q.z,
			2.0f * q.z * q.x - 2.0f * q.w * q.y, 0,
			2.0f * q.x * q.y - 2.0f * q.w * q.z,
			1.0f - 2.0f * q.x * q.x - 2.0f * q.z * q.z,
			2.0f * q.y * q.z + 2.0f * q.w * q.x, 0,
			2.0f * q.z * q.x + 2.0f * q.w * q.y,
			2.0f * q.y * q.z - 2.0f * q.w * q.x,
			1.0f - 2.0f * q.x * q.x - 2.0f * q.y * q.y, 0, 0, 0, 0, 1);
}

/* matrix functions */
static INLINE void m4_cons(mat4_t m, float m11, float m12, float m13, float m14,
