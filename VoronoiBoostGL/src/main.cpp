...
namespace polygon {
template<>
struct geometry_concept<biorobotics::Vertex2> {
	typedef point_concept type;
};

template<>
struct point_traits<biorobotics::Vertex2> {
	typedef int coordinate_type;

	static inline coordinate_type get(const biorobotics::Vertex2& vertex,
...
template<>
struct geometry_concept<biorobotics::Segment> {
	typedef segment_concept type;
};

template<>
struct segment_traits<biorobotics::Segment> {
	typedef int coordinate_type;
	typedef biorobotics::Vertex2 point_type;

	static inline point_type get(const biorobotics::Segment& segment,
			direction_1d dir) {
		return dir.to_int() ? segment.v1 : segment.v2;
	}
};
}
}
...
int add_primary_edges1(const voronoi_diagram<double>& vd) {
	int result = 0;
	for (voronoi_diagram<double>::const_edge_iterator it = vd.edges().begin();
		...
		if (it->is_primary()) {
			...
			if (it->is_finite()) {
				...
				vertexGL1.m_Pos = glm::vec3((float) it->vertex0()->x(),
						(float) it->vertex0()->y(), 0.0f);
				vertexDV.push_back(vertexGL1);
				vertexGL2.m_Color = glm::vec3(1.0f, 0.0f, 1.0f);
				vertexGL2.m_Pos = glm::vec3(it->vertex1()->x(),
						it->vertex1()->y(), 0.0f);
...
int main(int argc, char ** argv) {
	...
	std::vector<biorobotics::Segment> segments;
	for (int i = 0; i < polygons.size(); i++) {
		biorobotics::Polygon polygon = polygons[i];
		for (int j = 0; j < polygon.num_vertex; j++) {
			biorobotics::Segment segment;
			segment.v1 = polygon.vertex[j];
			if (j < polygon.num_vertex - 1)
				segment.v2 = polygon.vertex[j + 1];
			else
				segment.v2 = polygon.vertex[0];
...
// of edges around each cell (including secondary edges).
	{
		printf(
				"Number of edges (including secondary) around the Voronoi cells:\n");
...
// Linking Voronoi cells with input geometries.
	/*{
	 ...
	 biorobotics::Vertex2 v = points[index];
	 ...
	 printf(
	 "Cell #%ud contains segment start point: (%d, %d).\n",
	 ...
	 printf(
	 "Cell #%ud contains a segment: ((%d, %d), (%d, %d)). \n",
