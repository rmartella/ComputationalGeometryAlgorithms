...
int main(int argc, char ** argv) {
	...
	for (int i = 0; i < polygons.size(); i++) {
		...
		for (int j = 0; j < polygon.num_vertex; j++) {
			...
			vertexGL2.m_Pos = glm::vec3(vertex2.x, vertex2.y, 0.0f);
			vertexPolygon.push_back(vertexGL2);
			/*Site_2 t1(vertex1.x, vertex1.y);
			 vd.insert(t1);
			 Site_2 t2(vertex2.x, vertex2.y);
			 vd.insert(t2);*/
		}
	}
	...
	for (Edge_iterator it = vd.edges_begin(); it != vd.edges_end(); it++) {
		...
		if (it->is_segment()) {
			...
			vertexGL1.m_Pos = glm::vec3(it->source()->point().x(),
					it->source()->point().y(), 0.0f);
			vertexDV.push_back(vertexGL1);
			vertexGL2.m_Color = glm::vec3(1.0f, 0.0f, 1.0f);
			vertexGL2.m_Pos = glm::vec3(it->target()->point().x(),
					it->target()->point().y(), 0.0f);
