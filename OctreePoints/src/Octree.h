...
public:
	...
	Octree(const Octree& copy) :
			origin(copy.origin), halfDimension(copy.halfDimension), data(
					copy.data) {
		...
		if (isLeafNode()) {
			if (data == NULL) {
				...
				children[getOctantContainingPoint(oldPoint->getPosition())]->insert(
						oldPoint);
				children[getOctantContainingPoint(point->getPosition())]->insert(
						point);
			}
		} else {
	...
	void getPointsInsideBox(const Vec3& bmin, const Vec3& bmax,
