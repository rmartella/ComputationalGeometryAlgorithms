...
public:
	...
	template<class InCT1, class InCT2, template<class > class Point, template<
			class > class Segment>
	static typename enable_if<
			...
			void>::type discretize(const Point<InCT1>& point,
			const Segment<InCT2>& segment, const CT max_dist,
		...
		CT sqr_segment_length = segm_vec_x * segm_vec_x
		...
		const CT max_dist_transformed = max_dist * max_dist
				* sqr_segment_length;
...
private:
	...
	static typename enable_if<
			...
			CT>::type get_point_projection(const Point<CT>& point,
		...
		CT sqr_segment_length = segment_vec_x * segment_vec_x
