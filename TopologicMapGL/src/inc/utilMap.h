...
std::vector<Polygon> grownPolygons(std::vector<Polygon> polygons,
	...
	for (int i = 0; i < polygons.size(); i++) {
		...
		for (int j = 0; j < polygon.num_vertex; j++) {
			...
			/*std::cout << s.v1.x << "," << s.v1.y << std::endl;
			 std::cout << s.v2.x << "," << s.v2.y << std::endl;*/
			if (determinante1 > 0) {
				edgesGrownPolygons.push_back(s1);
				std::cout << s1.v1.x << "," << s1.v1.y << std::endl;
				std::cout << s1.v2.x << "," << s1.v2.y << std::endl;
			} else if (determinante2 > 0) {
				edgesGrownPolygons.push_back(s2);
				std::cout << s2.v1.x << "," << s2.v1.y << std::endl;
				std::cout << s2.v2.x << "," << s2.v2.y << std::endl;
			}
		}
					...
					for (int k = 0; k < polygon2.num_vertex; k++) {
						...
						for (int l = 0; l < polygons.size(); l++) {
							...
							for (int m = 0; m < polygonTest.num_vertex; m++) {
								...
								float d1 = getDeterminant(verticeTest1,
										verticeTest2, vertex1);
								float d2 = getDeterminant(verticeTest2,
										gverticeTest2, vertex1);
								float d3 = getDeterminant(gverticeTest2,
										gverticeTest1, vertex1);
								float d4 = getDeterminant(gverticeTest1,
										verticeTest1, vertex1);

								float d5 = getDeterminant(verticeTest1,
										verticeTest2, vertex2);
								float d6 = getDeterminant(verticeTest2,
										gverticeTest2, vertex2);
								float d7 = getDeterminant(gverticeTest2,
										gverticeTest1, vertex2);
								float d8 = getDeterminant(gverticeTest1,
										verticeTest1, vertex2);
...
bool ** addNodesInitEndToMap(Vertex2 init, Vertex2 end, Polygon * polygons,
		Polygon * grownPolygons, int sizePolygons, Vertex2 * vertexMap,
		bool ** adyacencies, int * sizeAdjacenciesPtr, float ratio) {
	...
	for (int i = 0; i < *sizeAdjacenciesPtr - 2; i++) {
		...
		for (int k = 0;
			...
			for (int l = 0;
				...
				float d2 = getDeterminant(vertex2, gverticeTest2,
						vertexTargetBind);
				float d3 = getDeterminant(gverticeTest2, gverticeTest1,
						vertexTargetBind);
				float d4 = getDeterminant(gverticeTest1, vertex1,
						vertexTargetBind);
