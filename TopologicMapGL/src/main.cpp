...
/*
 #include "simulator/simulation/parserFileWRL.h"
 #include "simulator/simulation/utilMap.h"
 #include "simulator/simulation/create_map.h"
 */

#include "inc/parserFileWRL.h"
#include "inc/utilMap.h"
...
std::vector<VertexXYZColor> grownvertexPolygon;
std::vector<VertexXYZColor> map;
...
int main(int argc, char* argv[]) {
	...
	std::vector<biorobotics::Polygon> polygons = biorobotics::parserFile(
			"filesWRL/random_2.wrl");
	std::vector<biorobotics::Polygon> grownPolygons =
			biorobotics::grownPolygons(polygons, ratio);
	for (int i = 0; i < polygons.size(); i++) {
		...
		for (int j = 0; j < polygon.num_vertex; j++) {
			...
			biorobotics::Vertex2 vertex1 = polygons[i].vertex[j];
			...
			if (j < polygons[i].num_vertex - 1) {
				vertex2 = polygons[i].vertex[j + 1];
			} else {
				vertex2 = polygons[i].vertex[0];
			}
	...
	for (int i = 0; i < grownPolygons.size(); i++) {
		biorobotics::Polygon polygon = grownPolygons[i];
		for (int j = 0; j < polygon.num_vertex; j++) {
			VertexXYZColor vertexGL1;
			VertexXYZColor vertexGL2;
			biorobotics::Vertex2 vertex1 = grownPolygons[i].vertex[j];
			biorobotics::Vertex2 vertex2;
			vertexGL1.m_Color = glm::vec3(1.0f, 0.0f, 0.0f);
			vertexGL1.m_Pos = glm::vec3(vertex1.x, vertex1.y, 0.0f);
			grownvertexPolygon.push_back(vertexGL1);
			if (j < grownPolygons[i].num_vertex - 1) {
				vertex2 = grownPolygons[i].vertex[j + 1];
			} else {
				vertex2 = grownPolygons[i].vertex[0];
			}
			vertexGL2.m_Color = glm::vec3(1.0f, 0.0f, 0.0f);
			vertexGL2.m_Pos = glm::vec3(vertex2.x, vertex2.y, 0.0f);
			grownvertexPolygon.push_back(vertexGL2);
		}
	}

	bool ** adyacencies;
	int sizeAdyacencies = 0;
	adyacencies = biorobotics::computeMapTopologic(polygons, grownPolygons,
			ratio, &sizeAdyacencies);

	biorobotics::Vertex2 * vertexMap;
	int sizeVertexMap = 0;
	vertexMap = biorobotics::createVertexPointerFromPolygons(&grownPolygons[0],
			grownPolygons.size(), &sizeVertexMap);
	...
	for (int i = 0; i < sizeAdyacencies; i++) {
		for (int j = 0; j < sizeAdyacencies; j++) {
			if (adyacencies[i][j] && adyacencies[j][i]) {
				VertexXYZColor vertexGL1;
				VertexXYZColor vertexGL2;
				vertexGL1.m_Color = glm::vec3(0.0f, 1.0f, 0.0f);
				vertexGL1.m_Pos = glm::vec3(vertexMap[i].x, vertexMap[i].y,
						0.0f);
				vertexGL2.m_Color = glm::vec3(0.0f, 1.0f, 0.0f);
				vertexGL2.m_Pos = glm::vec3(vertexMap[j].x, vertexMap[j].y,
						0.0f);
				map.push_back(vertexGL1);
				map.push_back(vertexGL2);
			}
		}
	}
	...
	float endticks = (float) (g_CurrentTicks - g_PreviousTicks);
	std::cout << "endticks=" << endticks << std::endl;
	...
	glBufferData(GL_ARRAY_BUFFER,
			grownvertexPolygon.size() * sizeof(VertexXYZColor),
			grownvertexPolygon.data(),
			GL_STATIC_DRAW);
	glVertexAttribPointer(positionAtribID, 3, GL_FLOAT, GL_FALSE,
			sizeof(VertexXYZColor), MEMBER_OFFSET(VertexXYZColor, m_Pos));
	/* Se activa el atributo de vertice*/
	...
	glBufferData(GL_ARRAY_BUFFER, map.size() * sizeof(VertexXYZColor),
			map.data(),
			GL_STATIC_DRAW);
	glVertexAttribPointer(positionAtribID, 3, GL_FLOAT, GL_FALSE,
			sizeof(VertexXYZColor), MEMBER_OFFSET(VertexXYZColor, m_Pos));
...
void DisplayGL() {
	...
	glBindVertexArray(g_vaos[1]);
	glDrawArrays(GL_LINES, 0, grownvertexPolygon.size());

	glBindVertexArray(g_vaos[2]);
	glDrawArrays(GL_LINES, 0, map.size());
