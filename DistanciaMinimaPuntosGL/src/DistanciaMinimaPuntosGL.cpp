...
void Initialize(int argc, char* argv[]) {
	...
	std::vector<glm::vec3> modelLineMin;
	...
	for (unsigned int i = 0; i < puntosConvert.size(); i++) {
		modelPoints.push_back(
				glm::vec3(puntosConvert[i].x * escala,
						puntosConvert[i].y * escala, 0));
	...
	modelLineMin.push_back(
			glm::vec3((minimo.p1.x) * escala, minimo.p1.y * escala, 0.0f));
	modelLineMin.push_back(
			glm::vec3((minimo.p2.x) * escala, minimo.p2.y * escala, 0.0f));

	CreateVBO(modelPoints, modelLineMin);
